...
_M["01"]._encode = function(addr, fc, startnum, reglen)
    return string.pack(">I6", 0) .. string.pack(">I1", addr) .. string.pack(">I1", fc) .. string.pack(">I2", startnum) .. string.pack(">I2", reglen)
end

-- 01功能码返回报文解析，IO点数量
_M["01"]._decode = function(response_mes, inputs)
    local datas = string.sub(response_mes, 10)
...
-- 03功能码返回报文解析，IO点数量，所有的点类型一致（int16,uint16,int32, uint32, float, double, string）
...
_M["05"]._encode = function(addr, fc, startnum, value)
...
_M["06"]._encode = function(addr, fc, startnum, value)
    ...
    local pack_bin =  string.pack(">I6", 0) .. string.pack(">I1", addr) .. string.pack(">I1", fc) .. string.pack(">I2", startnum) .. valuebin
    return pack_bin
end
